...
if(isset($_POST['simpan'])){
    ...
    $keter=$_POST['keter'];
    $tanggal=date("Y-m-d");
    $mysql=mysql_query("update rkb_edp_".$ctgl." set recid='1' status='$status',keter='$keter',tanggal='$tanggal' where kdtk='$kdtk'");
    if($mysql){
...
if(isset($_POST['ubah_jadwal'])){
    $kdtk=$_POST['kdtk'];
    $nama=$_POST['nama'];
    $tgl_rkb=$_POST['tgl_rkb'];
    $tgl_skrng=date("Y-m-d");
    $mysql=mysql_query("update rkb_edp_".$ctgl." set tgl_rkb='$tgl_rkb',tanggal='$tanggal' where kdtk='$kdtk'");
    if($mysql){
        echo ("<script>alert('Data Berhasil Ubah')</script>");
    }else{
...
<div id="templatemo_container">
    ...
    <div id="templatemo_content_wrapper">
        ...
        <div class="content_title_02">Laporan Kerja <?php echo $nama_id?></div>
            ...
            if($_POST['cari']){
                $cari=$_POST[cari];
                $sql=mysql_query("select b.* from area_edp_".$ctgl." a left join rkb_edp_".$ctgl." b on a.kdtk=b.kdtk where a.id='$id'");
            }else{
                $sql=mysql_query("select b.* from area_edp_".$ctgl." a left join rkb_edp_".$ctgl." b on a.kdtk=b.kdtk where a.id='$id'");
            }   
                $no=1;
                ...
                <tr bgcolor="<?=$warna?>">
                    ...
                    <td><?php if($row[recid]!='1'){
                        echo "<a href='jadwal_rkb_edit.php?kdtk=$row[kdtk]' title='Ubah Jadwal Kunjungan'>Ganti Jadwal</a></td>";}?>
                </tr>
                <?php }?>
