...
<form class="form-horizontal" method="POST" action="save" name="hitungOnline" >
	...
	<div class="form-group">
		...
		<div <div class="col-sm-9">
			<select name="nama">
				<?php
					$list_reporting = $this->db->query("SELECT kode,nama,jenis,dateline FROM list_reporting WHERE kode NOT IN(SELECT kode FROM reporting)")->result();
					foreach ($list_reporting as $row) {
						echo "<option value='$row->kode-$row->nama-$row->jenis-$row->dateline'>$row->nama - (Tgl $row->dateline)</option>";
					}
				?>
			</select>
		</div>
	...
	<div class="form-group">
		...
		<div <div class="col-sm-9">
			<input type="text" name="nilai" id="nilai" ng-model="nilai" size="8" value="0">
		</div>
	</div>
	<div class="form-group">
