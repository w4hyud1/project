	...
	<?php
		...
		//ADD FORM
			echo "<tr>
					<td>";
					foreach ($recod as $row) {
						echo "<input type='hidden' value='$row->nik' name='nik'>";
						echo "<input type='text' nama='nama' value='$row->nama'></td>";
						echo "</select></td>";
							for($i=1;$i<=$jbln;$i++){
								$tgl = "tgl_".$i;
								...
								echo "<option name='tgl_$i'></option>";
									foreach ($recod_shift as $row3) {
										if($row->$tgl==$row3->kd_shift){
											echo "<option name='".$row->$tgl."' selected=''>".$row->$tgl."</option>";	
										}else{
